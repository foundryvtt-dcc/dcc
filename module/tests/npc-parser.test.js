...
/* Test Snake */
const parsedSnake = parseNPC(superSnake);
test('expect name to be "Very long, the power super-snake"', () => {
  expect(parsedSnake.name).toBe("Very long, the power super-snake");
});
test('expect init to be "+0"', () => {
  expect(parsedSnake["data.attributes.init.value"]).toBe("+0");
});
test('expect AC to be "13"', () => {
  expect(parsedSnake["data.attributes.ac.value"]).toBe("13");
});
test('expect HP to be "21"', () => {
  expect(parsedSnake["data.attributes.hp.value"]).toBe("21");
});
test('expect Speed to be "20’"', () => {
  expect(parsedSnake["data.attributes.speed.value"]).toBe("20’");
});
test('expect Speed to be "1d20"', () => {
  expect(parsedSnake["data.attributes.actionDice.value"]).toBe("1d20");
});
test('expect Fort Save to be "+8"', () => {
  expect(parsedSnake["data.saves.frt.value"]).toBe("+8");
});
test('expect Ref Save to be "+4"', () => {
  expect(parsedSnake["data.saves.ref.value"]).toBe("+4");
});
test('expect Will Save to be "+4"', () => {
  expect(parsedSnake["data.saves.wil.value"]).toBe("+4");
});
test('expect Alignment to be "l"', () => {
  expect(parsedSnake["data.details.alignment"]).toBe("l");
});
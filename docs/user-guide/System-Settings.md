...
**Enable death clock for PCs** implements the death and dying countdown for player characters. When a PC drops to 0 hit points they gain a "Dying" condition with their level in rounds remaining — the combat tracker shows the countdown next to their name, and each combat round burns one round from the clock. If the clock reaches zero the character dies: they get the dead status overlay and are marked defeated in the combat tracker (exactly as if the judge had clicked the tracker's skull button), and the death is announced in chat. Healing the character above 0 HP before then clears the clock — and healing a dead PC above 0 revives them, removing the dead status and defeated marker. Level 0 characters die immediately at 0 HP, per the funnel rules. The countdown is also visible on the Dying condition itself, and a judge can apply or remove the Dying condition manually from the token HUD.

...
| `dcc.getSidebarTools` | `module/sidebar-tab.mjs` | `xcc` (planned: Mojo tracker) | §2.11 (relieves module-extension pressure) | Fires each time the DCC right-hand sidebar tab (issue #833) assembles its tool list, with the mutable `tools` record — `{ [name]: { label, icon, onClick } }` (`label` is an i18n key, `icon` Font Awesome classes). Listeners add, modify, or remove entries; core seeds Fleeting Luck (setting-gated) and Spell Duel. Fires on every tab render, so gate dynamic entries inside the listener. Stable from day one. |

...
surface so module authors know what they can rely on and so the adapter
over `@moonloch/dcc-core-lib` preserves the right behavior.
...
Cross-reference pass performed 2026-04-19; pain-point
...
## Stated contract: Foundry-smelling surface

The DCC system must keep its
...
| `dcc.afterSpellCheckResult` | `module/spell-check-processor.mjs:291` (legacy `processSpellCheck`) and `module/actor/spell-result-hook.mjs:61` (`emitAfterSpellCheckResult`, the shared emitter for every adapter terminal: `_castNakedViaAdapter`, `_castViaCastSpell`, `_castViaCalculateSpellCheck`, `_skillTableViaAdapter`) | `mcc-classes` (`module/patron-taint.mjs:66`, `module/glowburn.mjs:69`) | §2.4 (hardcoded wizard/cleric magic system), §2.11 (relieves module-extension pressure) | Post-result seam, fired once per spell check *and* per skill-table / disapproval-range skill check (Turn Unholy, Lay on Hands, Divine Aid) after the result is computed and the chat card rendered. **Informational** — listeners observe the outcome, they do not alter the already-rendered message; the attack-side counterpart `dcc.modifyAttackRollTerms` is the mutating one. Payload `(actor, {roll, item, naturalRoll, total, result, crit, fumble, success, castingMode, patronTaint, suppressPatronTaint, spellburn})`. Two fields are path-dependent: `result` carries the drawn Foundry `TableResult` whenever the path drew one — legacy `processSpellCheck`, the skill-table terminal, and (since #923) the adapter spell-check terminals for a spell that configures a results table — and is `null` for a cast with no table, which renders the pass/fail verdict from the lib's tier instead; `patronTaint` is non-null only on the legacy path, since the adapter routes taint through the lib's RAW pipeline (`onPatronTaint` in `adapter/spell-events.mjs`). A listener keying off a natural 1 (MCC's patron-taint trigger) should use `naturalRoll` rather than either. Added 2026-05-27, after the 2026-05-18 re-audit — this row was backfilled 2026-08-02 along with the fix that restored the emission from `_skillTableViaAdapter`, which the Phase 0–7 adapter rewrite had dropped. |
| `dcc.getSidebarTools` | `module/sidebar-tab.mjs` | core Death Clock tracker (`module/death-clock-tracker.mjs`, first in-core consumer); `xcc` (planned: Mojo tracker) | §2.11 (relieves module-extension pressure) | Fires each time the DCC right-hand sidebar tab (issue #833) assembles its tool list, with the mutable `tools` record — `{ [name]: { label, icon, onClick, help? } }` (`label` is an i18n key, `icon` Font Awesome classes, `help` an optional user-guide URL rendered as a link beside the button). Listeners add, modify, or remove entries; core seeds Fleeting Luck (setting-gated) and Spell Duel. Fires on every tab render, so gate dynamic entries inside the listener. Stable from day one. |
...
|---|---|---|---|---|
| `game.dcc.DCCRoll` | class | `dcc-qol` (`createRoll`), `xcc` (`createRoll`, `cleanFormula`) | §2.5 (extension surface), §2.9 (lib retirement reduces upgrade tax) | The unified roll abstraction. `createRoll(terms, data, options)` and `cleanFormula(terms)` are load-bearing. **Cancellation contract (2026-08-02, issue #867):** with `showModifierDialog: true`, closing the dialog without submitting rejects the returned promise with a `RollCancelledError` (`module/roll-cancellation.mjs`) — it used to reject with a bare `null`, which made a user cancel indistinguishable from a crash and blew up any `catch (err) { … err.message }`. Consumers should treat a cancel as a no-op: `import { rollOrNullOnCancel } from 'systems/dcc/module/roll-cancellation.mjs'` and `const roll = await rollOrNullOnCancel(game.dcc.DCCRoll.createRoll(…)); if (!roll) return`, or duck-type on `err?.isRollCancellation === true` (safe across module copies, unlike `instanceof`). |
...
| `game.dcc.processSpellCheck` | function | `xcc` (`xcc-actor-sheet.js` for wizard + cleric spells) | §2.4 (magic system), §2.5, §2.11 | Post-roll spell-check orchestrator: pre-built `Roll` + optional `RollTable` → patron taint check + crit/fumble classification + result-table lookup + level-added crit totaling + `SpellResult` chat render + wizard spell loss / cleric disapproval gating. **Phase 2 close (2026-04-18) finalized this as a permanent stable API** — not a deprecation target. **As of #923 (2026-09-08) the system itself no longer calls it on the spell-check path**: `DCCItem.rollSpellCheck` is a forwarder onto `DCCActor.rollSpellCheck`, and the adapter terminals gained result-table rendering (via `loadSpellResultsTable` + `game.dcc.SpellResult.addChatMessage`), the `system.lastResult` write, and the disapproval-range die annotation, so every core cast — sheet button, macro, magic item — resolves through the dispatcher. Remaining in-system callers are the skill-table spell checks (Turn Unholy, Lay on Hands, divine aid) in `rolls-skill-mixin.mjs`. It stays published and supported for sibling content modules (XCC's sheet paths with elf-trickster / blaster tweaks call it directly) and for any caller holding a pre-built `Roll`; treat it as the orchestrator for callers outside the dispatcher, not as a route the system falls back to. |
...
| `game.dcc.buildMightyDeedPrompt` | function | `dcc-qol` (attack card) | §2.5 (extension surface), §2.11 | `(message)` — return the Mighty Deed table prompt markup (`<select>` of configured deed tables + a "Roll Deed" button) for an attack `ChatMessage`, or `''` when the message has no `system.deedTables` to offer. The markup is self-describing (`data-deed-roll` on the container, the table path on each `<option>`), so it carries everything the click handler needs. Exists because modules that rebuild the attack card's `.message-content` (notably `dcc-qol`'s `enhanceAttackRollCard`, which replaces it wholesale and would otherwise clobber the system-rendered prompt) need to re-render the identical prompt inside their own card while leaving the lookup logic owned by the system. Source: `module/chat.js`. Added 2026-06-20 (issue #319 / dcc-qol compatibility). |
| `game.dcc.attachMightyDeedListeners` | function | `dcc-qol` (attack card) | §2.5 (extension surface), §2.11 | `(message, html)` — wire the system's Mighty Deed lookup handler to every `.roll-deed-table` button found within `html`. Idempotent (already-wired buttons carry a `data-deed-listener-attached` marker and are skipped), so the system render hook and a card-replacing module can both call it on the same DOM without double-binding (which would double-roll). The handler derives its source message/actor from the closest `[data-message-id]` element rather than the `message` argument, so it works regardless of which module rendered the prompt. Pairs with `buildMightyDeedPrompt`. Source: `module/chat.js`. Added 2026-06-20 (issue #319 / dcc-qol compatibility). |
| `game.dcc.buildSpellburnTerm` | function | `xcc` (target — its two class-sheet copies apply the burn with a raw `actor.update`, so an XCC spellburn produces no ability score log entry, no chat card and no Heal button) | §2.4 (magic system), §2.5, §2.11 | `(actor, { source?, onBurn? })` — build the Roll Modifier dialog's `Spellburn` term for a caster, with a callback that applies the committed burn on submit. `source` names the spell in the ability score log entries; `onBurn(total, commitment)` observes the points burned (MCC glowburn's patron manifestation keys off the amount). The term carries the caster `level`, which is what makes the dialog offer the "also adjust hit points" row (#921) — a hand-rolled term without it silently loses the feature. Push the returned object into the `terms` array handed to `game.dcc.DCCRoll.createRoll`. Source: `module/spellburn.mjs`. Added 2026-09-08 (issue #923, which consolidated the three independent spellburn implementations). |
| `game.dcc.applySpellburn` | function | (none yet — stable from day one per recommendation 7) | §2.4, §2.5 | `(actor, scores, source?, { adjustHP? })` — apply a spellburn directly, for callers that collect the commitment themselves rather than through a dialog term. `scores` are POST-burn ability scores (`{str, agl, sta}`), not burn amounts — `scoresFromBurnAmounts(actor, amounts)` in the same module converts. Writes the abilities and the optional Stamina ΔHP in one update, records the ability score log entries when the world setting is on, posts the chat card, and surfaces a rejected update to the player. Never rejects, so a synchronous dialog callback can fire-and-forget it. Source: `module/spellburn.mjs`. Added 2026-09-08 (issue #923). |
...
DCC system's shim removal. DCC removed the shim (2026-04-20);

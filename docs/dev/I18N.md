...
Copy the structure from `en.json` and translate all keys. Use `pnpm run compare-lang` to verify complete coverage.
...
5. **Run comparison**: Use `pnpm run compare-lang` to verify coverage

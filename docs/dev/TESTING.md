...
- **`dataPath` matters per checkout.** If you keep more than one checkout or worktree of the system (e.g. under `/Users/timlwhite/FoundryVTT/Data/systems/dcc` and `/Users/timlwhite/FoundryVTT-Next/Data/systems/dcc`), pointing the CLI at the wrong dataPath silently loads the OTHER copy of the system and your code changes don't show up. Verify by `curl http://localhost:30000/systems/dcc/module/actor.js | head` and check for an expected recent edit.
...
All functional specs use a **worker-scoped `sessionPage` fixture**: each worker logs into Foundry **once** and reuses the same page across every test, instead of re-navigating to `/join` + re-booting the system per test. This cut the e2e suite from ~840 s to ~340 s (e.g. `extension-api` 239 s → 34 s). `adapter-dispatch.spec.js` is the reference implementation. Implications when writing or editing these specs:
...
- **`cleanWorldState` auto-fixture (anti-bleed backstop).** `createSessionTest` also installs a test-scoped **auto** fixture that snapshots the actor list before each test and, afterwards, clears `game.user` targets + token selection and deletes any actors the test created (plus those actors' tokens on every scene). This protects later tests from a spec that threw before its own cleanup — stray tokens perturb token-distance rules (range / firing-into-melee / friendly fire) and leftover targets perturb target-driven rolls. Your per-test cleanup is still expected; the fixture is a backstop, not a replacement.
- **Canvas-probe specs must create a scene only when none exists and must NOT delete it.** The canvas-dependent probes (`weapon-range`, `friendly-fire`, `enhanced-attack-card`, `auto-apply-damage`) do `if (!game.canvas?.scene) { create + view }` and leave the scene in place so it is reused. Deleting the probe scene re-inits the canvas mid-run, briefly nulls `game.canvas.dimensions`, and `measureTokenDistance` returns `Infinity` — which flaked the live-token distance tests. The `cleanWorldState` fixture deliberately deletes actors/tokens but **not** scenes for the same reason.
- **`ui.windows` does NOT close ApplicationV2 dialogs under V14.** It is the AppV1 registry and is empty, so the usual `for (const app of Object.values(ui.windows)) await app.close()` cleanup leaves a `RollModifierDialog` (or any AppV2) open — it then survives into later tests and settles its promise there, attributing a console line to the wrong test. This is what made `adapter-dispatch.spec.js › two-weapon primary` fail ~50% of full-file runs (issue #867). Close AppV2 dialogs explicitly via `foundry.applications.instances`, scoped by a class the dialog owns (e.g. `app.options.classes.includes('roll-modifier')`) so the core UI apps in the same registry are left alone. Note also that pressing `Escape` does not reliably close a dialog that isn't focused — don't rely on `fireAndForget` for cleanup.
- **Live-token timing.** When a test needs a token's *placeable* (not just its document) for a canvas rule, await **every** placeable it depends on (e.g. both target and ally), not just one — under the loaded shared session a second placeable can lag. Likewise, `fromUuid` of a freshly-created embedded item can lag a tick; poll it before driving a handler that resolves by uuid.
